                                ...
                                info_array_with_verbal_commands* array_vb_cmd)
{
    ...
    #define _SIZE_OF_THE_COMMAND_ "11"
    ...
    CHECK_ERROR(commandProcessing(&ip, command, NUMBER_OF_LABELS, array_nl_cmd, lables_for_assembler,
                                  array_vb_cmd, &fixup));
                               ...
                               info_array_with_verbal_commands* array_vb_cmd, stack_info* fixup)
{
    ...
    while(characters_were_read < array_vb_cmd->size_of_the_array_with_verbal_commands)
    {
        ...
        sscanf(array_vb_cmd->array_with_verbal_commands + characters_were_read,
               "%" _SIZE_OF_THE_COMMAND_ "s%n", command, &command_size);
        ...
        {
            CHECK_ERROR(labelProcessing(command, NUMBER_OF_LABELS, ip, lables_for_assembler));
        }
        else
        {
            fprintf(stdout, "%s\n", command);
        ...
        CHECK_ERROR(skipSpaces(array_vb_cmd, &characters_were_read));
    }
